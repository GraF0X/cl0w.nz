    ...
    v.innerHTML = `
       ...
       <script type="module">
        ...
        function init() {
            ...
            bloomPass = new UnrealBloomPass( new THREE.Vector2( screenWidth, screenHeight ), 1.5, 0.4, 0.85 );
            ...
            hBlurPass.uniforms[ 'h' ].value = postProcessingParams.blurSize / ( screenWidth * pixelRatio );
            vBlurPass.uniforms[ 'v' ].value = postProcessingParams.blurSize / ( screenHeight * pixelRatio );
            ...
            fxaaPass.material.uniforms[ 'resolution' ].value.x = 1 / ( screenWidth * pixelRatio );
            fxaaPass.material.uniforms[ 'resolution' ].value.y = 1 / ( screenHeight * pixelRatio );
            ...
            asciiEffect.setSize( screenWidth, screenHeight );
            ...
            const modelUrl = 'https://dl.dropboxusercontent.com/scl/fi/0gdxk6nilh77qvy1zk4fb/Xbot.glb?rlkey=xc55bdbaz3t4gx5y6ocs2amob&st=pq29j9j1';
            ...
            loader.load( modelUrl, function ( gltf ) {
                ...
                // Ініціалізація IK - зроблена безпечнішою
        ...
        function initIK( mesh ) {
            ...
            // Безпечна функція пошуку індексу кістки
            const getIndex = (names) => {
                for (let name of names) {
                    if (boneMap[name] !== undefined) return boneMap[name];
                }
                // Пошук за частковим збігом (для надійності)
                for (let name of names) {
            ...
            chainConfigs.forEach( config => {
                ...
                // Якщо кістка не знайдена або індекс некоректний, пропускаємо цей ланцюжок
        ...
        function createMergedGUI() {
            ...
            // --- ПАПКА 1: ВІЗУАЛЬНІ НАЛАШТУВАННЯ ---
            ...
            ikFolder.add( ikConfig, 'enabled' ).name('Enable IK').onChange( val => {
                ...
                // Перевірка існування skinnedMesh перед використанням
                if (val && skinnedMesh && skinnedMesh.skeleton) {
                    const bones = skinnedMesh.skeleton.bones;
                    // Оскільки ми додаємо цілі в кінець масиву bones, обчислюємо зсув
                    // Але краще не покладатися на це при повторному вмиканні, тому що позиції вже можуть бути змінені
                }
            });
            ...
            // Налаштування для кісток (стандартна анімація)
            panelSettings = {
            ...
            // Відкриваємо папки за замовчуванням
            ikFolder.open();
            visualFolder.close();
            lightFolder.close();
            folder1.open();
        ...
        function animate() {
            ...
            if ( spotLight ) {
                if ( lightConfig.animate ) {
                    ...
                    // Синхронізація ручних контролів
